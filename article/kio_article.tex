% Статья для журнала «Компьютерные инструменты в образовании» (КИО)
% Основано на материалах ВКР: METELKINMA_repository_for-the_polytechnic_thesis/VKR/
% Кодировка файла: UTF-8
\documentclass[intlimits,twoside,a4paper,11pt]{article}

\usepackage[utf8]{inputenc}

% Шаблон журнала (kioj4.sty предоставлен руководителем)
\usepackage[eqsecnum]{kioj4}

% Доп. пакеты (минимально необходимые)
\usepackage{array}
\usepackage{graphicx}
...
% Год известен, номер выпуска неизвестен
\issue{2026}{-}{edu}

% УДК (уточняется редакцией при необходимости)
\udknumber{004.4}

% Название: краткий вариант в [] и вариант в верхнем регистре в {}
...
\autocitationexample
% \acknowledgements Работа выполнена в рамках выпускной квалификационной работы по направлению «Прикладная информатика».
\end{abstract}
...
Сравнение выполнено для трех инструментов поствалидации: Great Expectations (GX), SodaCL и dbt tests. Для обеспечения функциональной сопоставимости используются идентичные логические правила (реализованные через SQL-запросы, подсчитывающие число нарушений) и единый набор данных. Эксперимент включает базовые прогоны и прогоны с контролируемыми изменениями, имитирующими типовые дефекты данных на этапах ETL; результаты агрегируются по повторным запускам. Для каждого прогона фиксируются метрики качества обнаружения (наличие нарушений и число проваленных проверок) и метрики стоимости выполнения (длительность, CPU, RAM), регистрируемые в техническом контуре наблюдаемости.
