...
def run_stg_completeness_suite(engine, dag_id: str, run_id: str, parent_run_id: str, payloads: Dict[str, Any]):
    from app2.validators.runner import run_validation
    cfg = load_config("STG")
    layer_cfg = cfg.get("layers", {}).get("STG", {}) if isinstance(cfg, dict) else {}
    suite_cfg = layer_cfg.get("suites", {}).get("completeness_suite", {}) if isinstance(layer_cfg, dict) else {}
    validations_cfg = layer_cfg.get("validations", {}) if isinstance(layer_cfg, dict) else {}
    if not suite_cfg.get("enabled", True):
        return

    entity_name = suite_cfg.get("entity", "STG_completeness_validation_suite")
    ...
    try:
        for validator_name in suite_validations:
            ...
            if not v_cfg.get("enabled", True):
                continue
            if validator_name not in payloads:
                continue
            payload = payloads[validator_name]
        ...
        audit_log(
            ...
            status="SUCCESS",
            message=f"Completeness suite completed, validations run: {count}",
            started_at=start_dt,
        ...
        audit_log(
            ...
            entity_name=entity_name,
            status="FAILED",
            message=f"Completeness suite failed: {e}",
            started_at=start_dt,
            finished_at=datetime.now(),
        )
        finish_validation_run(
