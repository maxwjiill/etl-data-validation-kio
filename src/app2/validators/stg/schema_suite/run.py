...
def run_stg_schema_suite(engine, dag_id: str, run_id: str, parent_run_id: str, payloads: Dict[str, Any]):
    from app2.validators.runner import run_validation
    cfg = load_config("STG")
    layer_cfg = cfg.get("layers", {}).get("STG", {}) if isinstance(cfg, dict) else {}
    suite_cfg = layer_cfg.get("suites", {}).get("schema_suite", {}) if isinstance(layer_cfg, dict) else {}
    validations_cfg = layer_cfg.get("validations", {}) if isinstance(layer_cfg, dict) else {}
    if not suite_cfg.get("enabled", True):
        return

    entity_name = suite_cfg.get("entity", "STG_schema_validation_suite")
    ...
    try:
        ...
        audit_log(
            ...
            status="SUCCESS",
            message=f"Schema suite completed, validations run: {count}",
            started_at=start_dt,
        ...
        audit_log(
            ...
            entity_name=entity_name,
            status="FAILED",
            message=f"Schema suite failed: {e}",
            started_at=start_dt,
            finished_at=datetime.now(),
        )
        finish_validation_run(
