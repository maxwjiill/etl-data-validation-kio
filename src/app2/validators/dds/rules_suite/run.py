...
def run_dds_rules_suite(engine, dag_id: str, run_id: str, parent_run_id: str, conn=None):
    from app2.validators.runner import run_validation

    cfg = load_config("DDS")
    layer_cfg = cfg.get("layers", {}).get("DDS", {}) if isinstance(cfg, dict) else {}
    suite_cfg = layer_cfg.get("suites", {}).get("rules_suite", {}) if isinstance(layer_cfg, dict) else {}
    validations_cfg = layer_cfg.get("validations", {}) if isinstance(layer_cfg, dict) else {}
    if not suite_cfg.get("enabled", True):
        return

    entity_name = suite_cfg.get("entity", "DDS_rules_validation_suite")
    ...
    try:
        ...
        audit_log(
            ...
            status="SUCCESS",
            message=f"DDS rules suite completed, validations run: {count}",
            started_at=start_dt,
        ...
        audit_log(
            ...
            entity_name=entity_name,
            status="FAILED",
            message=f"DDS rules suite failed: {e}",
            started_at=start_dt,
            finished_at=datetime.now(),
        )
        finish_validation_run(
