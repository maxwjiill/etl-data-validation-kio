...
def run_dds_referential_suite(engine, dag_id: str, run_id: str, parent_run_id: str, conn=None):
    ...
    suite_cfg = layer_cfg.get("suites", {}).get("referential_suite", {}) if isinstance(layer_cfg, dict) else {}
    validations_cfg = layer_cfg.get("validations", {}) if isinstance(layer_cfg, dict) else {}
    if not suite_cfg.get("enabled", True):
        return

    entity_name = suite_cfg.get("entity", "DDS_referential_validation_suite")
    ...
    try:
        for validator_name in suite_validations:
            ...
            if not v_cfg.get("enabled", True):
                continue
            payload = {"engine": engine, "conn": conn, "run_id": run_id}
        ...
        audit_log(
            ...
            status="SUCCESS",
            message=f"Referential suite completed, validations run: {count}",
            started_at=start_dt,
        ...
        audit_log(
            ...
            entity_name=entity_name,
            status="FAILED",
            message=f"Referential suite failed: {e}",
            started_at=start_dt,
            finished_at=datetime.now(),
        )
        finish_validation_run(
